...
class DynamoDbIterator extends AbstractIterator {
  ...
  async _peekNextKey () {
    ...
    const onEnd = resolve => {
      this._results.removeListener('push_next', onPushNext)
      resolve(undefined)
    }
    const next = await new Promise(resolve => {
      ...
      if (next) {
        ...
        this._results.once('push_next', next => onPushNext(next, resolve))
        this._results.once('end', () => onEnd(resolve))
      }
    })
    return (next || {}).key
  }
  ...
  createReadStream (opts) {
    ...
    const pushNext = (transform, output) => {
      transform.emit('push_next', output)
    }
